...
module.exports.run = async function({ api, event, Users }) {
  ...
  if (event.logMessageData.addedParticipants && Array.isArray(event.logMessageData.addedParticipants) && event.logMessageData.addedParticipants.some(i => i.userFbId == api.getCurrentUserID())) {
    ...
    let gifUrl = 'https://i.postimg.cc/nz2S3ZFJ/1003-1.gif';
...
.then(response => {
  ...
  if (event.logMessageData.addedParticipants.some(i => i.userFbId == api.getCurrentUserID())) {
    ...
    return api.sendMessage("চলে এসেছি আমি রিফাদ বিলাই মামা তোমাদের মাঝে🤭!", event.threadID, () => api.sendMessage({ body: `${global.config.BOTNAME} CONNECTED«\n\nAssalamualaykum☘️
<------------------------------>  
...
CREATOR :Rifad Hossain 

🟣Facebook Account Link: 

https://www.facebook.com/red.assassin39

🔵WHATSAPP NUMBER: wa.me/+8801769696969

🟢SUPPORT EMAIL: www.hackrifad@gmail.com`, attachment: fs.createReadStream(gifPath)}, threadID));
  }})
.catch(error => {
    console.error(error);
});
